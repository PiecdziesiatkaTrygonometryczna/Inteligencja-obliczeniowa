...
def prepare_datasets(test_size, validation_size):
    ...
    valid_indices = np.isin(y, np.arange(len(GENRE_LABELS)))
    X = X[valid_indices]
    y = y[valid_indices]
...
if __name__ == "__main__":
    ...
    cm = confusion_matrix(y_test, y_pred_labels, labels=np.arange(len(GENRE_LABELS)))

    # Plot confusion matrix
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=GENRE_LABELS)
    disp.plot(cmap=plt.cm.Blues)
    plt.title("Confusion Matrix")
    plt.savefig("../plots/confusion_matrix5")
